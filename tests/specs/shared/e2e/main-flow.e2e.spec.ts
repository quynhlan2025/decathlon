...
test.describe('Main Flow – Homepage to Purchase @regression @p1 @main-flow', () => {
  ...
  test('TC-MAIN-001: homepage loads successfully', async ({ pages, region }) => {
  ...
  test('TC-MAIN-004: user can navigate from search results to PDP', async ({ buyer, pages, region }) => {
    await buyer.searchProduct('running');
    const product = ProductFactory.getRandomProduct(region, 'running');
  ...
  test('TC-MAIN-005: product detail page displays price', async ({ pages, region }) => {
    ...
    expect(price).toMatch(product.expectedPricePattern);
  });

  // ─── 3. PDP → Add to Cart ─────────────────────────────────────

  test('TC-MAIN-006: user can add product to cart from PDP', async ({ buyer, pages, region }) => {
    const product = ProductFactory.getRandomProduct(region);
    await buyer.addProductToCart(product.url);
    await pages.cart.goto();
    const count = await pages.cart.getItemCount();
    expect(count).toBeGreaterThan(0);
  });

  test('TC-MAIN-007: cart is not empty after adding a product', async ({ buyer, pages, region }) => {
    const product = ProductFactory.getRandomProduct(region);
    await buyer.addProductToCart(product.url);
    await pages.cart.goto();
  ...
  test('TC-MAIN-008: cart item count increases after adding product', async ({ buyer, pages, region }) => {
    const product = ProductFactory.getRandomProduct(region);
    await pages.cart.goto();
  ...
  test('TC-MAIN-009: cart page loads and shows checkout button', async ({ buyer, pages, region }) => {
    const product = ProductFactory.getRandomProduct(region);
    await buyer.addProductToCart(product.url);
    await pages.cart.goto();
  ...
  test('TC-MAIN-010: cart persists after page reload', async ({ buyer, pages, region }) => {
    const product = ProductFactory.getRandomProduct(region);
    await buyer.addProductToCart(product.url);
    await pages.cart.goto();
  ...
  test('TC-MAIN-011: user can remove item from cart', async ({ buyer, pages, region }) => {
    const product = ProductFactory.getRandomProduct(region);
    await buyer.addToCartAndGoToCart(product.url);
    const countBefore = await pages.cart.getItemCount();
  ...
  test('TC-MAIN-012: cart shows total price', async ({ buyer, pages, region }) => {
    const product = ProductFactory.getRandomProduct(region);
    await buyer.addToCartAndGoToCart(product.url);
  ...
  test('TC-MAIN-014: full flow — category browse to add-to-cart', async ({ buyer, pages, region }) => {
    await pages.home.gotoCategory('RUNNING');
    const product = ProductFactory.getByCategory(region, 'running') ?? ProductFactory.getRandomProduct(region);
    await buyer.addProductToCart(product.url);
    await pages.cart.goto();
    const count = await pages.cart.getItemCount();
    expect(count).toBeGreaterThan(0);
  ...
  test('TC-MAIN-015: guest can proceed to checkout from cart', async ({ buyer, pages, region }) => {
    const product = ProductFactory.getRandomProduct(region);
    await buyer.addToCartAndGoToCart(product.url);
  ...
  test('TC-MAIN-016: guest checkout completes with valid data', async ({ buyer, region }) => {
  ...
  test('TC-MAIN-017: order confirmation page shown after guest checkout', async ({ buyer, region }) => {
    const data = CHECKOUT_TEST_DATA[region];
    const product = ProductFactory.getRandomProduct(region);
    await buyer.addProductToCart(product.url);
  ...
  test('TC-MAIN-018: logged-in user can complete purchase @logged-in', async ({ buyer, region }) => {
    const product = ProductFactory.getRandomProduct(region, 'running');
    await buyer.buyProduct(product.url);
    await expect(buyer.getPage()).toHaveURL(/confirmation|success|thank/i);
  });
